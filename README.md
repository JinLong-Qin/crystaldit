...
#! 因为安装版本没有POT_GGA_PAW_PBE目录，所以构建一个软链接
ln -s PBE POT_GGA_PAW_PBE
...
```bash
# Batch generation for all checkpoints  # 将所有保存的模型都计算出1000个晶体结构  
./eval_script/batch_generatefor.sh ./output/crystal_dit 1000 32

# Calculate balance scores and select best 3 models    # 把计算的结果绘制可视化图   checkpoint_evaluation_results 是上一步生成晶体的同时生成的分析目录， analysis
...
python generate_crystals.py 
    --checkpoint ./output/mattpuscibert_processed_text3/checkpoint_epoch_2750.pt 
    --num_samples 10000 
    --batch_size 32 
    --output_dir ./generated_crystals_text3 
    --use_multi_gpu
```
...
# example
python eval_script/chgnet_process.py 
    --cif_folder ./test_crystals/checkpoint_epoch_20750_generated 
    --csv_file ./test_crystals_flowmm_aligned_analysis/checkpoint_epoch_20750_generated_results.csv 
    --mp_hull_path ./mp_02072023/2024-08-07-ppd-mp.pkl 
    --sample_size 500 
    --output_dir ./chgnet_results
```
...
./eval_script/run_vasp_all.sh /ssd1/CrystalDiT/chgnet_results_text3 32
```
...
# input-dir要具体到df_un目录下  如： chgnet_results_text3/dft_un  和 chgnet_results_text3/dft_non_un
# output-dir应该也要具体到df_un下， 如： chgnet_results_text3/clean_outputs
python eval_script/create_traj_for_ehull.py \
    --input-dir ./vasp_results \   
    --output-dir ./clean_outputs    

python eval_script/create_traj_for_ehull.py 
    --input-dir ./chgnet_results_text3/dft_non_un 
    --output-dir ./chgnet_results_text3/clean_outputs_text3
```
...
python eval_script/dft_post_processor.py 
    --chgnet_csv ./chgnet_results_text3/chgnet_results.csv 
    --dft_results_folder ./chgnet_results_text3 
    --original_csv_file ./mattpuscibert_processed_text3_crystals_flowmm_aligned_analysis/checkpoint_epoch_2750_generated_results.csv 
    --mp_hull_path ./mp_02072023/2024-08-07-ppd-mp.pkl 
    --output_dir ./final_results_text3
```
