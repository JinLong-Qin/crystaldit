...
def preprocess_dataset(csv_file, output_file, max_atoms=20, max_length=46.7425):
    ...
    try:
        for idx, row in tqdm(data.iterrows(), total=len(data), desc="Preprocessing progress"):
            ...
            # todo -------------------------------------- Done   修改了文本，现在只有一个text文本，里面包含了少量结构信息和大部分属性信息
            text = row['text'] 
            text_vectors = get_text_emb(text)
            '''
            # !编码的维度有问题，需要768  Done
            structure_text = row['text']
            properties_text = row['processed_text2']
            # print(f'structure_text: {structure_text}')
            # print(f'properties_text: {properties_text}')

            text_emb_structure = get_text_emb(structure_text)
            text_emb_properties = get_text_emb(properties_text)
            print(f'preprocess_dataset ---------------------')
            print(f'text_emb_structure: {text_emb_structure.shape}')
            print(f'preprocess_dataset end---------------------')
            # todo --------------------------------------
            '''
            try:
                ''' 
                    # todo 1.增加文本特征的解析
                        # todo 1.1.增加matscibert，直接保存文本特征
                    # todo 2.在模型中增加特征融合部分
                        # todo 2.1.特征融合
                        # todo 2.2.双文本特征的前后学习
...
class CrystalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text_vectors = sample_data['text_vectors']
