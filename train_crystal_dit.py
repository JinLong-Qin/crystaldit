...
def train_epoch(model, diffusion, dataloader, optimizer, scheduler, device, epoch, global_step, args, rank):
    ...
    for step, batch in enumerate(iterator):
        ...
        '''
        text_vectors = batch["text_vectors"].to(device)
        lattice_vectors = batch["lattice_vectors"].to(device)
        atom_features = batch["atom_features"].to(device)
        
        optimizer.zero_grad()
        loss_dict = diffusion.training_losses(model, text_vectors, lattice_vectors, atom_features)
        loss = loss_dict["loss"]
        ...
        # print(f'-----------------------------------')
        # for name, p in model.module.named_parameters():  # 注意这里 model.module 因为用了DDP
        #     if p.requires_grad and p.grad is None:
        #         print(f"[rank {rank}] 没有梯度的参数: {name}")
        # print(f'-----------------------------------')
...
def train(rank, world_size, args):
    ...
    #     print(f"[rank {rank}] IDX {idx} NAME {name} requires_grad={p.requires_grad} shape={tuple(p.shape)}")
...
'''
python train_crystal_dit.py --data_dir ./datasets/mp_20 --output_dir ./output/mattpuscibert_properities_only --batch_size 256 --epochs 15000 --save_every 250 --hidden_size 512 --depth 18 --num_heads 8
5000及之前的模型生成结果都有问题   原子类别太多了，可能训练量不够
'''


'''
python eval_script/balance_score_calculator.py ./checkpoint_evaluation_results --alpha 1.0 --max_epoch 50000
'''