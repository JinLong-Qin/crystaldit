...
class GlobalDataManager:
    ...
    def __init__(self, main_folder: str, csv_folder: str, cpu_manager: CPUManager):
        '''main_folder: 要计算评估指标的cif文件夹
            _folder:参考数据己的root文件夹，包含train.csv, val.csv, test.csv
        '''
        ...
        self.all_structures = {}  # {folder_name/file_name: structure}  所有要计算的cif文件对象
        self.all_crystals = {}    # {folder_name/file_name: crystal}
    
    def load_reference_datasets(self):
        """Load reference datasets  把参考数据集都读进来"""
        print("="*60)
    ...
    def load_all_structures(self):
        """Load all CIF files at once  把所有需要计算评估指标的cif文件都读进来"""
        print("="*60)
    ...
    def convert_all_to_crystals(self):
        """Convert all Structures to Crystal objects at once  把所有读进来的结构文件 cif 转为晶体对象 SimplifiedCrystal"""
        print("="*60)
...
class BatchMetricsCalculator:
    ...
    def __init__(self, data_manager: GlobalDataManager, cpu_manager: CPUManager):
        ...
        # Preprocess reference datasets     计算评估指标使用的参考数据集
...
class OptimizedCrystalEvaluator:
    ...
    def __init__(self, main_folder: str, csv_folder: str):
        ...
        # Create managers
        self.cpu_manager = CPUManager()  # CPU管理器
        self.data_manager = GlobalDataManager(main_folder, csv_folder, self.cpu_manager) # 数据管理器
    ...
    def run_evaluation(self):
        ...
        # Stage 1-3: Global data loading  读取所有数据，包括参考数据集mp_20 ; 和待评估数据集 cif文件数据 并转换为 SimplifiedCrystal
        ...
        # Create batch calculator   计算评估指标的计算器对象
        ...
        for folder_name in tqdm(pending_folders, desc="Saving results"):
            try:
                ...
                python eval_script/balance_score_calculator.py \
                    ...
                    --alpha 1.0 \
                    --max_epoch 50000
                提示缺少json文件，应该是要先跑stage 4中的计算评估指标生成json的文件
                再回头去计算balance score   
                # !(如果stage 2中确实没有生成对应的评估json文件的话)
