...
def calculate_balance_score_bac(un_rate, structural_validity, chemical_validity, 
...
def calculate_balance_score(un_rate, structural_validity, chemical_validity, 
    ...
    - 1. modified the threshold for each component score calculation
    - 2. adding the eps to avoid exact zero collapse
    """
    
    # 1. Structural validity score  0.7-1
    struct_score = max(0, min(1, (structural_validity - 0.7) / (1.0 - 0.7)))
    
    # 2. Chemical validity score 0.6-1
    chem_score = max(0, min(1, (chemical_validity - 0.6) / (1.0 - 0.6)))
    
    # 3. Density distribution score (1.5-0.1 range, lower is better)
    density_score = max(0, min(1, (1.5 - density_diff) / (1.5 - 0.1)))
    
    # 4. Elements distribution score (1.0-0.1 range, lower is better)
    elements_score = max(0, min(1, (1.0 - elements_diff) / 0.9))
    
    # 5. Quality composite (geometric mean - any poor score hurts overall)
    # ! tiny eps to avoid exact zero collapse
    eps = 1e-8
    quality_composite = ((struct_score + eps) * 
                         (chem_score + eps) * 
                         (density_score + eps) * 
                         (elements_score + eps)) ** (1.0 / 4.0)
...
def process_results_folder(folder_path, alpha_values=None, max_epoch=50000):
    ...
    for json_file in folder_path.glob("*.json"):
        ...
        try:
            ...
            density_diff = data['distribution_metrics']['d_density_test']       
            elements_diff = data['distribution_metrics']['d_elements_test']     

            # !before batch_eval_metrics生成的内容是_test，但是这里默认是_train
            # density_diff = data['distribution_metrics']['d_density_train']       #d_density_test
            # elements_diff = data['distribution_metrics']['d_elements_train']     #d_elements_test 
