...
def train_epoch(model, diffusion, dataloader, optimizer, scheduler, device, epoch, global_step, args, rank):
    ...
    for step, batch in enumerate(iterator):
        structure_vectors = batch["structure_vectors"].to(device)
        properties_vectors = batch["properties_vectors"].to(device)
        lattice_vectors = batch["lattice_vectors"].to(device)
        atom_features = batch["atom_features"].to(device)
        
        # print(f'in train_epoch --------------------')
        # print(f'structure_vectors: {structure_vectors.shape}') # structure_vectors: torch.Size([256, 1, 31090])
        # print(f'properties_vectors: {properties_vectors.shape}') # properties_vectors: torch.Size([256, 1, 31090])
        # print(f'lattice_vectors: {lattice_vectors.shape}') # lattice_vectors: torch.Size([256, 3, 3])
        # print(f'atom_features: {atom_features.shape}') # atom_features: torch.Size([256, 20, 5])
        # print(f'in train_epoch end --------------------')
        
        optimizer.zero_grad()
        loss_dict = diffusion.training_losses(model, structure_vectors, properties_vectors, lattice_vectors, atom_features)
        loss = loss_dict["loss"]
...
def train(rank, world_size, args):
    ...
    if rank == 0:
        ...
        if args.force_preprocess or not os.path.exists(train_preprocessed):
            print(f'Process the data in {os.path.join(args.data_dir, "train.csv")}')
            preprocess_dataset(
    ...
    train_loader = DataLoader(
        ...
        # sampler=train_sampler, # multi-gpus
        shuffle=True,
...
'''
python train_crystal_dit_single.py --data_dir ./datasets/mp_20 --output_dir ./output/test --batch_size 256 --epochs 500 --save_every 25 --hidden_size 512 --depth 18 --num_heads 8
'''